...
test.describe('Chatbot Functionality', () => {
  ...
  test('should display chat interface elements when opened', async ({ page }) => {
    // Open chat
    await page.click('#chat-toggle');
    await page.waitForTimeout(500);
    ...
    await expect(page.locator('#chat-input')).toHaveAttribute('placeholder', 'Enter your question here');
  });
  ...
  test('should validate minimum message length', async ({ page }) => {
    ...
    await expect(errorMessage).toContainText('Please enter a message with at least 10 characters');
  });
  ...
  test('should send message and receive response', async ({ page }) => {
    ...
    await page.waitForSelector('.loading-bubble', { state: 'detached', timeout: 45000 });
    ...
    await expect(botResponse).toContainText(/developer|Andrew|professional|software|web/i);
  });
  ...
  test('should show loading animation while waiting for response', async ({ page }) => {
    // Open chat
    await page.click('#chat-toggle');
    await page.waitForTimeout(500);
  ...
  test('should sanitize user input', async ({ page }) => {
    ...
    const maliciousMessage = '<script>alert("test")</script>What does Andrew do?';
  ...
  test('should scroll messages into view', async ({ page }) => {
    ...
    for (let i = 1; i <= 3; i++) {
      await page.fill('#chat-input', `Test message number ${i} - What does Andrew Ford do professionally?`);
      await page.click('#chat-send');
